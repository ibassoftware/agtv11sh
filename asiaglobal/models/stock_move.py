...
class StockMove(models.Model):
    ...
    def _get_landed_cost(self):
        for record in self:
            ...
            move_ids = self.env['stock.move'].search(
                [('product_id', '=', record.product_id.id), ('state', '=', 'done'), ('remaining_qty', '>', 0)])
            for move in move_ids:
                if move.location_id.usage == 'supplier' and move.location_dest_id.usage == 'internal':
                    landed_cost_line = self.env['stock.valuation.adjustment.lines'].search(
                        [('move_id', '=', move.id)])
    ...
    product_id_code = fields.Char(
        string='Item Code', compute='_get_product_details')
    product_id_partno = fields.Char(
        string='Part Number', compute='_get_product_details')
    product_id_description = fields.Char(
        string='Description', compute='_get_product_details')
    location = fields.Char()
    has_landed_cost = fields.Boolean(
        string='With LC?', compute="_get_landed_cost")
    price_unit = fields.Float(
        'Unit Price', digits=(12, 5), help="Technical field used to record the product cost set by the user during a picking confirmation (when costing "
    ...
    def move_account_entry_move(self):
        for move in self:
            if not move.account_move_ids and move.value != 0:
                move.with_context(
                    force_period_date=move.date)._account_entry_move()
...
class StockMoveLine(models.Model):
    ...
    remaining_value = fields.Float(
        related="move_id.remaining_value", copy=False)
    price_unit = fields.Float(
